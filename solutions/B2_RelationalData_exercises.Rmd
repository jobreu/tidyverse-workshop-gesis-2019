...
For the dataset with the maximum amount of observations (rows) it does not matter which datset you start from. Name the resulting datasets *gap_nomiss* and *gap_full*.
```
...
```{block, opts.label = "clues"}
The *gapminder* dataset should be the key/reference/master table for this join.
```
