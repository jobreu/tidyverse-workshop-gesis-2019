...
# What is a pipe?
```{r, out.width = "40%"}
include_graphics("./pics/pipe office decoration.jpg")
```
...
Say, if we aim to calculate the mean value of the variable `mpg`, as smart people we can fire up this command:
...
We can use indentions to workaround issues with readability:
...
Another workaround for our problem is using curly brackets.
...
They are what you may know from other languages being called lambda-functions.
...
.center[<small><small>Source: https://github.com/rstudio/hex-stickers/blob/master/PNG/tibble.png </small></small>]
...
---
# tribbles
Using the `tribble`-approach, is a way to create `tibbles` for small tables as it resembles the final layout of the table.
...
2. tibbles
  ...
  - the standard format of the `tidyverse`
