...
```{r echo=TRUE}
datasauRus::datasaurus_dozen %>% names()
```
...
datasauRus::datasaurus_dozen %>% group_by(dataset) %>%
  summarise(n = n(), first_x = first(x), first_y = first(y), 
            mean_x = mean(x), mean_y = mean(y), sd_x = sd(x),
            sd_y = sd(y), corr = cor(x, y, method = "pearson"))
```
